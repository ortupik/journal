...
const WordCloudChart = ({ data }: Props) => {
  ...
  const {
    ...
    const sorted = [...data].sort((a, b) => b.value - a.value).slice(0, 30); // Limit to top 30 words
    ...
    <Card className='col-span-2 md:col-span-2'>
      ...
      <CardContent className='p-4'>
        ...
        <div className='mb-4 grid grid-cols-3 gap-2 text-center'>
          <div>
            <p className='text-muted-foreground text-xs'>Total Occurrences</p>
            <p className='font-bold text-blue-600'>{totalWordOccurrences}</p>
          </div>
          <div>
            <p className='text-muted-foreground text-xs'>Top Word</p>
            <p className='font-bold text-purple-600'>{topWord.text}</p>
          </div>
          <div>
            <p className='text-muted-foreground text-xs'>Top Word Count</p>
        ...
        <div
          ...
          className='relative flex h-96 w-full items-center justify-center overflow-hidden rounded-lg bg-slate-50 p-8'
        >
          <AnimatePresence>
            {mounted &&
              sortedData.map((word, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className='group absolute transform'
                    style={{
                  ...
                  >
                    <span
                      className={` ${getColor(index)} cursor-default font-bold opacity-90 transition-all group-hover:opacity-100 hover:scale-110`}
                    >
                      {word.text}
                    </span>
                  </motion.div>
