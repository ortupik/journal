...
function EditJournal() {
  ...
  const debouncedContent = useDebounce(content, 20000);
  ...
  const { mutate: updateJournalEntry, isLoading: isUpdatingJournal } =
    useMutation({
  ...
  if (isUpdatingJournal || isFetching)
    ...
    <div className='mx-auto my-3 max-w-screen-lg'>
      ...
      >
        ...
        <div className='mt-6'>
          <button
            type='submit'
            className='w-xs rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none disabled:opacity-60'
